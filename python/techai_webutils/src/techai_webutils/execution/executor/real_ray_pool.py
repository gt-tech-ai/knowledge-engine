...
optional ``techai-webutils[ray]`` extra), so a caller imports it lazily only when the actor pool is
configured; the default asyncio path never loads it. This mirrors the
